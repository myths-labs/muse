...
MUSE 结合 Markdown 技能与 Python 运行器，支持有 Git、Python 3.7+
...
3.7.1 新增可选的运行环境预检，会在隔离条件下检查工作流实际选用的 Python，
避免普通终端里能找到的依赖在实际运行时不可用。日常保存和恢复仍只使用
Python 标准库。

明确需要图片解码的工作流，还可以验证并完整解码 PNG、JPEG 样例。Pillow
是可选依赖，安装 MUSE 不会自动安装它。这项检查不新增原生附件接入能力，
也不替代实际工作流测试。让代理按安装技能的
[运行环境预检说明](../skills/core/muse-commands/references/RUNTIME_DEPENDENCIES.md)
检查后，用同一个解释器重新验证真实工作流。
...
3.7 可以核验超过默认16MiB总量限制的完整登记范围，不需要丢弃文件。
接收代理在准备和接手两处显式选择 `full-scope-batches-v1`，审阅新证明后接手。
你仍然使用原来的 resume 指令，具体参数由代理处理。

每批实际读取最多8MiB，单文件最多5MiB，范围最多3000路径。全部路径经过两轮
内容读取，接手时在写入锁内重新读取核验。内容变化、证明缺失、未知协议和并发
过期写入者都会被拒绝；默认 legacy 限制保留。完整覆盖只针对登记范围及其明确
排除项，不代表范围外文件或产品已经验收。

两条命令与审阅输入见安装技能的[日常协议说明](../skills/core/muse-commands/references/DAILY_V1.md)。
准备和接手必须使用同一个协议。

## 检查点接近容量上限时

增量保存的输入、选定 source note 和检查点各有 256 KiB 上限（262,144 个
UTF-8 字节）。最终检查点还包含元数据和分隔符，不能只数文字或新增内容。
上面的工作区分批核验不会扩大这个容量。

详细测试结果和日志放在证据文件中；代理只把必要事实和简短引用存入检查点，
source note 本身也要符合读取上限。只有 `Next Action` 和 `Required Reads`
可以替换，历史决定、限制、未解决问题及其证据继续保留。

遇到 `TOO_LARGE` 时，先保留失败记录和证据。代理核对当前状态与写入者，
整理符合协议的较小增量，再通过受保护保存核验归档及真实检查点。仍放不下时，
保留未保存证据，另行审阅存储方案；不能靠扩大上限或删除历史让保存通过。
简短引用也不意味着检查点有无限容量。

完整保留与读回步骤见安装技能的
[检查点容量说明](../skills/core/muse-commands/references/CHECKPOINT_CAPACITY.md)。
本节说明既有行为，不新增自动历史压缩，也不改变 3.7.1 发行版本。
