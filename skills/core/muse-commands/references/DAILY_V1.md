...
Git evidence includes dirty tracked/untracked content and staged blob IDs, not just status/path names. The default `legacy-v1` protocol keeps a 5 MiB per-file limit, 16 MiB total content budget, at most 3000 paths and two nested-repository levels. Exceeding these bounds or encountering unreadable/unsupported content yields PARTIAL and refuses a new writer claim. Recovery still returns the checkpoint and uncertainty; do not automatically read oversized files or bypass the refusal. Git-ignored files and production environment state need their separate task obligations. This is a bounded Git-relevant observation, not a frozen filesystem snapshot.

For a Lane with an explicit task scope, the current writer may register reviewed files using `enable-daily ... --scope-file <json> --scope-sha256 <hash>` while pinning the checkpoint SHA. The scope schema has exactly schema_version=1, workspace, files (absolute regular/missing file paths within configured projects), excluded_work and authorization_ref. It must describe the user's actual authorized task, not manufacture permission. The helper hashes every selected file's bytes, including ignored files explicitly selected, plus Git refs/index/tracked status. Scope contents and exclusions are bound into git.sha256; scope/content changes invalidate an outstanding claim. COMPLETE then means complete **within that named scope**, not the whole product. The scope is a policy file beside the protocol marker, not a duplicate current/writer state. Prior scope bytes are preserved before a scope update.

### Explicit full-scope batches

For a registered scope larger than the legacy total budget, select the same protocol on both commands:

```bash
muse-doctor.sh prepare-resume --command '/resume strategy Lane A' --workspace /path/to/project --snapshot-protocol full-scope-batches-v1
muse-doctor.sh claim-lane --input /path/to/reviewed-claim.json --snapshot-protocol full-scope-batches-v1
```

The agent reviews the fresh preparation result and uses its `git.sha256` in the claim input above. Both commands default to `legacy-v1`; there is no automatic fallback. An unknown protocol or a missing registered scope is refused. Never trim the scope to obtain a successful claim.

Each deterministic batch permits at most 8 MiB of actual content reads, including failed reads; files remain limited to 5 MiB and scopes to 3000 paths. Strict reads use unbuffered handles and the remaining byte allowance, so a file growing after planning cannot overrun the batch budget. Every scope path, including declared absences, is covered in two full content passes. A failed or changed batch makes the entire observation PARTIAL.

Proofs bind exact membership, content, scope bytes, Git HEAD/branch/index, checkpoint, protocol marker, source revision and receiving native session. Claim performs fresh reads inside the checkpoint lock, checks the reviewed proof and current dependencies, then preserves all original sections and the handoff ID while appending the writer transition. Old writers and concurrent stale claims remain rejected. This verifies observable drift between cooperating clients; it does not provide an immutable filesystem snapshot or certify product readiness.
