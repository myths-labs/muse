...
9. 🔴🔴 **Default-Deny secret file inspect**（BUG-MUSE-19 v2 · 5/9 ship · 写死永久）: Agent 永不直接 inspect 任何 `.env*` / `~/.config/*.env` / `secrets.json` / `*.pem` / `*.key` · 不论命令多 length-only。永远禁止 `xxd` / `cat` / `head` / `tail` / `printf` / `echo` / `awk -F='...' FILE` (envless line plaintext leak) / `sed` / `wc` / `grep` / `tr` / `python3 -c "open(...)"` / `node -e "fs.readFileSync(...)"` / `Read` tool。需要 length / format / drift info → Agent 输出 command · JC terminal 跑 · JC paste plain text 数字给 Agent。例外: `Write FILE` (整体覆盖) / `open -a "Editor" FILE` (JC 自己 edit)。详见 `CLAUDE.md` `🔴 安全红线` 中的 Default-Deny subsection + `bye.md` Step 4.6。**Why structural fix**: BUG-MUSE-19 v1 (5/7 ship `xxd/cat/...` ban list) · 5/9 `awk -F=` 不在 ban list · default 信任 length-only OK · envless format leak Tripo token · ban list 模式失效 · v2 一次性 ban 全部 inspect 行为 · 不再依赖 ban list 维护。
